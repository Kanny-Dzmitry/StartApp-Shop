...
class OrderSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ['id', 'user', 'address', 'payment_method', 'status', 'created_at', 'total_price', 'items', 'comment']
...
class OrderCreateSerializer(serializers.Serializer):
    ...
    total_price = serializers.DecimalField(max_digits=10, decimal_places=2, required=False)  # Общая сумма заказа
